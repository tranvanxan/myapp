...
var loggedIn = FlowRouter.group({
    subscriptions: function(params) {
        ...
        this.register('Register_UserData', Meteor.subscribe('Channel_Images'));
    },
    triggersEnter: [function(context, redirect) {
...
//Tạo định tuyến root từ frontpageRoutes (tức là /)
...
//Tạo định tuyến image từ frontpageRoutes
frontpageRoutes.route('/cfs/files/:collectionName', {
    subscriptions: function(params) {
        this.register('Register_UserData', Meteor.subscribe('Channel_Images'));
    },
    triggersEnter: [function(context, redirect) {
...
frontpageRoutes.route('/cfs/files/:collectionName/:id', {
    subscriptions: function(params) {
        this.register('Register_UserData', Meteor.subscribe('Channel_Images'));
    },
    triggersEnter: [function(context, redirect) {
...
frontpageRoutes.route('/cfs/files/:collectionName/:id/filename', {
    subscriptions: function(params) {
        this.register('Register_UserData', Meteor.subscribe('Channel_Images'));
    },
    triggersEnter: [function(context, redirect) {
...
IsSystem = function() {
    if (FlowRouter.current().path.indexOf("/system")!=-1||FlowRouter.current().path.indexOf("/login")!=-1) {
        ...
        if ($('script[src="/system/languages/'+G_Lang.get()+'.js"]').length == 0) {
            ...
            InsertJsCssFile('/system/include/ckedit/ckeditor.js', 'js', '', false);
        }
    }
    else{
        if ($('link[href="/system/css/style.css"]').length > 0 ) {
